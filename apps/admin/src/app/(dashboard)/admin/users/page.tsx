...
import { MoreHorizontal, Shield, ShieldOff, Trash2, UserCog } from "lucide-react"
import { useRouter } from "next/navigation"
import { useEffect, useState } from "react"

import { useSession } from "@/lib/auth-client"

import { banUserAction, listUsersAction, removeUserAction, setRoleAction } from "./actions"
...
export default function UsersPage() {
	...
	const handleBanUser = async (userId: string, ban: boolean) => {
		...
		try {
			...
			if (result.success) {
				setUsers((prev) =>
					prev.map((user) => (user.id === userId ? { ...user, banned: ban } : user)),
				)
			}
		} catch (error) {
	...
	const handleSetRole = async (userId: string, role: "admin" | "super_admin") => {
		setActionLoading(userId)
		try {
