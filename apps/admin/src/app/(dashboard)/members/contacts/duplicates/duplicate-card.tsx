...
export function DuplicateCard({ group, onDeduplicate }: DuplicateCardProps) {
	const names = group.contacts.map((c) => `${c.firstName} ${c.lastName}`).join(", ")

	return (
		<Card>
			<CardHeader className="flex flex-row items-start justify-between gap-4 space-y-0 pb-3">
				<div className="space-y-1">
					<CardTitle className="font-heading text-base font-medium">{names}</CardTitle>
					<p className="text-sm text-muted-foreground">{group.contacts.length} contacts</p>
				</div>
			...
			<CardContent className="space-y-3">
				<div className="flex justify-end">
					<Button variant="secondary" size="sm" onClick={() => onDeduplicate(group)}>
						Deduplicate
					</Button>
				</div>
			</CardContent>
