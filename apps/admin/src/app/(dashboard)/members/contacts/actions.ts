...
export async function findDuplicatesAction(): Promise<
...
> {
  ...
  try {
    ...
    for (const [root, contacts] of groupMap) {
      ...
      const confidence =
        types.has("EMAIL") || types.has("PHONE") ? "HIGH" : "LOW";
  ...
  try {
    await db.transaction(async (tx) => {
      ...
      const sources = await tx
        .select()
        .from(contact)
        .where(inArray(contact.id, sourceIds));
      ...
      for (const sourceId of sourceIds) {
        ...
        for (const scc of sourceClubContacts) {
          if (targetClubIds.has(scc.clubId)) {
            ...
            await tx
              .delete(clubContactRole)
              .where(eq(clubContactRole.clubContactId, scc.id));
      ...
      const targetCommitteeKeys = new Set(
        targetCommittees.map((c) => `${c.role}|${c.homeClubId}`),
      );
